...
適用範圍:本節的純文字 code block 格式僅適用 Codex；末段三項誠實性要求同時適用 Codex 與 Claude。

不論工作是完成、進行中、暫停、失敗或等待人類裁定,每次對人類回報時都必須附上一段**可直接複製**的純文字 code block。不得因尚未達成 DoD、只是在回報 blocker、只完成部分工作或正在詢問決策而省略。

可複製報告必須依當下實際狀態詳細列出:

- 目前步驟與狀態
- 已完成的工作與修改檔案
- 尚未完成、刻意未做或跳過的工作,以及原因
- 真實執行結果、測試、coverage、Ruff、mypy 狀態；沒跑也要明講
- 遇到的問題、錯誤訊息、影響範圍與已採取的處理
- 需要人類決定的事項與建議選項
- Git 狀態:是否 commit、是否 push、目前 HEAD 或 commit
- 下一步與可重現命令(若適用)

報告內容不得把「之前通過」寫成「最新通過」,不得把未提交內容寫成已交付,也不得省略已知缺口。
